...
namespace SmartHotel.UI.Controllers
{
    public class RoomController : Controller
    {
        ...
        // Cấu hình JSON để tự động map chữ hoa/thường (roomId -> RoomId)
        ...
        {
            // Tạo client (nên đặt tên client trong Program.cs để bỏ qua SSL nếu cần)
       ...
       string? roomNumber = null)
        ...
        public async Task<IActionResult> Create(CreateRoomRequest request)
        {
            ...
            {
                var client = await CreateClientAsync();
                var response = await client.PostAsJsonAsync("Room", request); // Chỉ gọi "Room"
