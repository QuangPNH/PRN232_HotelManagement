...
namespace SmartHotel.UI.Controllers
{
    public class TenantController : Controller
    {
        ...
        // Đã sửa tên Constructor cho đúng với TenantController
        ...
        // --- 1. HELPER: Tạo Client và gắn Token ---
        private async Task<HttpClient> CreateClientAsync()
        {
            var client = _httpClientFactory.CreateClient("SmartHotelClient");
            client.BaseAddress = new Uri(_apiSettings.AuthBaseUrl); // Cảm ơn bạn đã lưu ý URL này, nhớ đảm bảo nó trỏ đúng tới Base URL của API (ví dụ: https://localhost:xxxx/api/)
        ...
        // --- 2. XEM DANH SÁCH (Phân trang & Lọc) ---
        public async Task<IActionResult> Index(string? keyword, bool? isActive, int pageIndex = 1, int pageSize = 10)
        {
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    // Lưu lại tham số tìm kiếm để hiển thị lại trên View
        ...
        // --- 4. XỬ LÝ TẠO MỚI KHÁCH HÀNG THỦ CÔNG ---
        [HttpPost]
        ...
        // --- 5. KHÓA / MỞ KHÓA / DUYỆT TÀI KHOẢN ---
        [HttpPost]
        public async Task<IActionResult> ToggleStatus(int id)
        {
            ...
            try
            {
                var response = await client.PutAsync($"Tenant/{id}/toggle-status", null); // Body truyền null vì không cần data
            ...
            // Reload lại trang danh sách (Referer giúp quay lại đúng trang và tham số tìm kiếm hiện tại)
